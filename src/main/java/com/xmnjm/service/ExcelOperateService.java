...
public class ExcelOperateService {
    ...
    public void setProduct(Products products, Row row, Integer scenarioWhat) {
        ...
        //品牌
        if (row.getCell(1) != null && StringUtils.hasText(row.getCell(1).toString())) products.setBrand(row.getCell(1).toString());
        //产品名称
        if (row.getCell(2) != null && StringUtils.hasText(row.getCell(2).toString())) products.setProductName(row.getCell(2).toString());
        //ASIN
        if (row.getCell(3) != null && StringUtils.hasText(row.getCell(3).toString())) products.setAsin(row.getCell(3).toString());
        //分类
        if (row.getCell(4) != null && StringUtils.hasText(row.getCell(4).toString())) products.setProductTypeName(row.getCell(4).toString());
        //价格
        if (row.getCell(5) != null && StringUtils.hasText(row.getCell(5).toString())) {
            try {
                products.setPrice(Double.valueOf(row.getCell(5).toString()));
            } catch (Exception e) {

            }
        }
        //BuyBox价格
        if (row.getCell(6) != null && StringUtils.hasText(row.getCell(6).toString()) && "-".equals(row.getCell(6).toString())) {
            try {
                products.setBuyBoxPrice(Double.valueOf(row.getCell(6).toString()));
            } catch (Exception e) {
        ...
        if (row.getCell(7) != null && StringUtils.hasText(row.getCell(7).toString())) {
            try {
                products.setOffer(Double.valueOf(row.getCell(6).toString()));
            } catch (Exception e) {
        ...
        if (row.getCell(8) != null && StringUtils.hasText(row.getCell(8).toString())) {
            try {
                products.setCommentNumber(Integer.valueOf(row.getCell(8).toString()));
            } catch (Exception e) {
        ...
        if (row.getCell(9) != null && StringUtils.hasText(row.getCell(9).toString())) {
            ...
            if (kind.length > 0) products.setProductSize(kind[0]);
            if (kind.length > 1) products.setProductColour(kind[1]);
        }

        //国家
        if (row.getCell(11) != null && StringUtils.hasText(row.getCell(11).toString())) products.setCountry(row.getCell(11).toString());

        //产品网址
        if (row.getCell(13) != null && StringUtils.hasText(row.getCell(13).toString())) products.setProductUrl(row.getCell(13).toString());

        //缩略图地址
        if (row.getCell(14) != null && StringUtils.hasText(row.getCell(14).toString())) products.setProductThumbnail(row.getCell(14).toString());

        //高清图地址
        if (row.getCell(15) != null && StringUtils.hasText(row.getCell(15).toString())) products.setProductImageUrl(row.getCell(15).toString());

        //备注
        if (row.getCell(17) != null && StringUtils.hasText(row.getCell(17).toString())) products.setComment(row.getCell(17).toString());

        //产品短描述
        if (row.getCell(18) != null && StringUtils.hasText(row.getCell(18).toString())) products.setShortDescription(row.getCell(18).toString());

        //产品长描述
        if (row.getCell(19) != null && StringUtils.hasText(row.getCell(19).toString())) products.setLongDescription(row.getCell(19).toString());

        //Date First Available
        if (row.getCell(22) != null && StringUtils.hasText(row.getCell(22).toString())) products.setDataFirstAvailable(row.getCell(22).toString());

        //Best Sellers Rank
        if (row.getCell(23) != null && StringUtils.hasText(row.getCell(23).toString())) products.setBestSellersRank(row.getCell(23).toString());
    }
